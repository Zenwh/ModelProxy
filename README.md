...
`feishu-relay` 系统的 **Gateway** 端 — 给独立仓 [Zenwh/feishu-relay-bot](https://github.com/Zenwh/feishu-relay-bot) 的 bot 节点提供心跳接入、请求中转、客户端 API key 管理。对外暴露 OpenAI / Anthropic 兼容 API，把请求经飞书消息隧道派发给内网 bot 节点。
...
```
   ...
   ───────────►   │                                          │
                   ...
                   └──────────────┬───────────────────────────┘
                                  ...
                                  ▼
                          ┌───────────────────┐
                          │  feishu-relay-bot │  ← 独立仓:
                          │   (内网员工机器)   │    Zenwh/feishu-relay-bot
                          │                   │
                          │  POST /v1/messages│
                          └─────────┬─────────┘
                                    │
...
跟 bot 节点之间走 `relay_protocol v2`（详见 [feishu-relay-bot/feishu_relay_bot/relay_protocol.py](https://github.com/Zenwh/feishu-relay-bot/blob/main/feishu_relay_bot/relay_protocol.py)）。所有消息走飞书 IM 文本隧道，超 50KB 自动 zlib 压缩。
